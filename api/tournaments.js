...
function generateRoundRobinBracket(participants, doubleRound = true) {
  const matches = [];
  const n = participants.length;
  ...
  // RETOUR (Rounds rounds+1 à 2*rounds) — inverser les joueurs
  if (doubleRound) {
    // Réinitialiser pour la seconde passe
    shiftedPlayers.length = 0;
    for (let i = 0; i < players.length; i++) shiftedPlayers.push(players[i]);
    shiftedPlayers.shift(); // Remove fixed player again
    ...
    for (let r = 1; r <= rounds; r++) {
      ...
      for (let m = 1; m <= matchesPerRound; m++) {
        ...
        if (p1 && p2) {
          // ⚠️ IMPORTANT: Inverser p1 et p2 pour le match retour
          matches.push({
            round: r + rounds,
            match_number: matchNum++,
            bracket_type: 'round_robin',
            player1_id: p2,  // Inversé
            player2_id: p1,  // Inversé
            player1_from: 'seed',
            player2_from: 'seed',
            status: 'pending'
          });
...
function setupTournamentRoutes(app, pool, auth, io) {
  ...
  app.post('/tournaments', auth, async (req, res) => {
    ...
    try {
      ...
      const result = await pool.query(`
        ...
        RETURNING *
        `, [name, description, format, status || 'draft', max_participants, third_place_match || false, req.user.uid]);
  ...
  app.delete('/tournaments/:id', auth, async (req, res) => {
    ...
    try {
      ...
      // 🔧 FIX: Delete GUEST players BEFORE tournament cascade-delete
      // This ensures orphan GUEST cleanup works correctly
      const guestParticipants = await client.query(`
        SELECT DISTINCT tp.player_id
        FROM tournament_participants tp
        WHERE tp.tournament_id = $1
          AND tp.player_id LIKE 'guest_%'
      `, [id]);

      const guestPlayerIds = guestParticipants.rows.map(r => r.player_id);
      ...
      // Now delete orphan GUEST players that are no longer referenced
      let orphanGuests = { rows: [] };
      if (guestPlayerIds.length > 0) {
        orphanGuests = await client.query(`
          DELETE FROM players
