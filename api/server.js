...
/* ================================================================
   BACKUP SYSTEM — Pure Node.js (sans pg_dump / sans disque)
...
async function generateSqlDump() {
  ...
  lines.push(`-- GOUZEPE backup ${stamp}`);
  lines.push(`-- Generated by Node.js (no pg_dump)`);
  ...
  for (const seq of seqRes.rows) {
    ...
    // Trouve la valeur max actuelle
    try {
      const cur = await q(`SELECT last_value FROM "${sname}"`);
      if (cur.rowCount) lines.push(`SELECT setval('${sname}', ${cur.rows[0].last_value}, true);`);
    } catch (_) {}
  }
...
async function restoreFromSqlString(sql) {
...
async function listBackups() {
  // Combine: fichiers disque (si dispo) + backups_store en DB
  const result = [];

  // Depuis la DB
  try {
    const r = await q(`SELECT id, name, size_bytes, source, created_at FROM backups_store ORDER BY created_at DESC`);
  ...
  // Depuis le disque local (fallback Docker)
  try {
    const entries = fs.readdirSync(BACKUP_DIR, { withFileTypes: true })
      .filter(e => e.isFile() && e.name.endsWith('.sql') && !e.name.startsWith('uploaded_'))
      .map(e => {
        const st = fs.statSync(path.join(BACKUP_DIR, e.name));
        return { name: e.name, sizeBytes: st.size, updatedAt: st.mtime.toISOString(), source: e.name.startsWith('auto_') ? 'auto' : 'manual', storage: 'disk' };
      });
    result.push(...entries);
  } catch (_) {}
...
async function createDatabaseBackup(kind = 'manual') {
  ...
  try {
    ...
    const fileName = `${kind}_${stamp}.sql`;

    const sql = await generateSqlDump();
    const sizeBytes = Buffer.byteLength(sql, 'utf8');

    // Stocker en DB (fonctionne sur Render + Neon)
    await q(`
      INSERT INTO backups_store(name, size_bytes, source, data, created_at)
      VALUES($1, $2, $3, $4, now())
    `, [fileName, sizeBytes, kind, sql]);

    // Pruning auto: garde 10 backups auto max
    if (kind === 'auto') {
      await q(`
        DELETE FROM backups_store WHERE id IN (
          SELECT id FROM backups_store WHERE source='auto'
          ORDER BY created_at DESC OFFSET 10
        )
      `);
    }

    // Aussi sur disque si disponible (local Docker)
    try {
      fs.writeFileSync(path.join(BACKUP_DIR, fileName), sql, 'utf8');
    } catch (_) {}

    return { name: fileName, sizeBytes, updatedAt: new Date().toISOString(), source: kind };
  } finally {
    backupJobRunning = false;
  }
}

async function restoreFromBackupName(fileName) {
  if (backupJobRunning) throw new Error('Une operation de sauvegarde/restauration est deja en cours');
  backupJobRunning = true;
  try {
    // Cherche en DB d'abord
    let sql = null;
    const dbRow = await q(`SELECT data FROM backups_store WHERE name=$1 LIMIT 1`, [fileName]);
    if (dbRow.rowCount) {
      sql = dbRow.rows[0].data;
    } else {
      // Fallback disque
      const filePath = path.join(BACKUP_DIR, sanitizeSqlFileName(fileName) || '');
      if (fs.existsSync(filePath)) sql = fs.readFileSync(filePath, 'utf8');
    }
    if (!sql) throw new Error('Backup introuvable: ' + fileName);
    await restoreFromSqlString(sql);
  } finally {
    backupJobRunning = false;
  }
}
...
async function ensureSchema(){
  ...
  /* table stockage backups en DB (Render/Neon compatible, sans disque) */
  await q(`CREATE TABLE IF NOT EXISTS backups_store (
    id         SERIAL PRIMARY KEY,
    name       TEXT NOT NULL,
    size_bytes INTEGER,
  ...
  await q(`CREATE INDEX IF NOT EXISTS idx_backups_store_created ON backups_store(created_at DESC)`);
...
// Téléchargement — sert depuis DB ou disque
app.get('/admin/backups/:fileName/download', auth, adminOnly, async (req, res) => {
  try {
    const fileName = sanitizeSqlFileName(req.params.fileName);
    if (!fileName) return bad(res, 400, 'Nom invalide');
    ...
    if (dbRow.rowCount) {
      ...
      return res.send(dbRow.rows[0].data);
    }

    // Fallback disque
    const fullPath = path.join(BACKUP_DIR, fileName);
    if (fs.existsSync(fullPath)) return res.download(fullPath, fileName);

    bad(res, 404, 'Backup introuvable');
  } catch (e) {
    bad(res, 500, e.message || 'Erreur téléchargement');
  }
});
...
app.delete('/admin/backups/:fileName', auth, adminOnly, async (req, res) => {
  try {
    ...
    // Supprime en DB
    await q(`DELETE FROM backups_store WHERE name=$1`, [fileName]);

    // Supprime sur disque si présent
    const fullPath = path.join(BACKUP_DIR, fileName);
    if (fs.existsSync(fullPath)) fs.unlinkSync(fullPath);

    ok(res, { ok: true, deleted: fileName });
  } catch (e) {
...
app.post('/admin/backups/restore-existing', auth, adminOnly, async (req, res) => {
  try {
    ...
    await restoreFromBackupName(fileName);
    ok(res, { ok: true, restoredFrom: fileName });
  } catch (e) {
...
app.post('/admin/backups/restore-upload', auth, adminOnly, restoreUpload.single('sqlFile'), async (req, res) => {
  try {
    ...
    ok(res, { ok: true, restoredFrom: req.file.originalname });
  } catch (e) {
